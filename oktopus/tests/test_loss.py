import pytest
import numpy as np
from ..loss import L1Norm, L2Norm
...
@pytest.mark.parametrize("data", ([np.random.normal(size=200)],
                                  [np.random.poisson(size=200)]))
def test_L2Norm_median_estimate(data):
    l2norm = L2Norm(data=data, model=lambda t: t)
    result = l2norm.fit(x0=(np.mean(data)), method='L-BFGS-B')
    assert abs(result.x - np.median(data)) / np.median(data) < 1e-1


@pytest.mark.parametrize("data", ([np.random.normal(size=200)],
                                  [np.random.poisson(size=200)]))
def test_L2Norm_median_estimate_w_regularization(data):
    l2norm = L2Norm(data=data, model=lambda t: t,
                    regularization=GaussianPrior(mean=np.median(data),
                                                 var=np.std(data)**2))
    result = l2norm.fit(x0=(np.mean(data), 0.5), method='L-BFGS-B')
    assert abs(result.x[0] - np.median(data)) / np.median(data) < 1e-1
